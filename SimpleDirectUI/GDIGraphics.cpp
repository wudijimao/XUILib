...
GDIGraphics::GDIGraphics() :mIsCreated(false)
{
...
void GDIGraphics::Creat(HWND hwnd)
{
	mHwnd = hwnd;
	//初始化画布
	HDC hdc = ::GetDC(hwnd);
...
void GDIGraphics::ReSize(int width, int height)
{
	mWidth = width;
	mHeight = height;
}

void GDIGraphics::Destory()
{
	if (mIsCreated)
	{
		mIsCreated = false;
...
void GDIGraphics::DrawBackGround(Gdiplus::Color &color, const XResource::XRect &r)
{
...
void GDIGraphics::DrawImage(Gdiplus::Bitmap &img, const XResource::XRect &rect, int x, int y)
{
...
void GDIGraphics::DrawImage(XResource::XImage &xImg, const XResource::XRect &xRect)
{
	XRect r = xImg.GetDrawRect(xRect);
	mBkgGraphics->DrawCachedBitmap(xImg.CacheImage(*mBkgGraphics), r.X(), r.Y());
}

void GDIGraphics::DrawString(const std::wstring &text, const XResource::XRect &xRect)
{
...
void GDIGraphics::DrawString(const std::wstring &text, const XResource::XRect &xRect, Gdiplus::StringAlignment align)
{
...
void GDIGraphics::DrawString(const XResource::XText &text, const XResource::XRect &xRect)
{
...
XRect GDIGraphics::MeasureString(const std::wstring &text)
{
	RectF temp;
	FontFamily fontFamily(L"Arial");
	Font font(&fontFamily, 12, FontStyleBold, UnitPoint);
...
XRect GDIGraphics::MeasureString(const XResource::XText &text, const XResource::XRect &xRect)
{
	FontFamily   fontFamily(text.mFont.c_str());
	Font         font(&fontFamily, text.FontSize, text.mFontStyle, UnitPoint);
	StringFormat stringFormat;
...
bool GDIGraphics::Init(IXCanvas *canvas)
{

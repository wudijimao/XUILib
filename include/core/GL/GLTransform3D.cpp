...
GLTransform3D::GLTransform3D(const GLTransform3D &left, const GLTransform3D &right) {
    const GLfloat *l = left._transformMat;
    const GLfloat *r = right._transformMat;
    int numLeft = 0;
    int numRight = 0;
    for (int i = 0; i < 15; ++i) {
...
void GLTransform3D::setScale(GLfloat x, GLfloat y) {
    _transformMat[0] = x;
    _transformMat[5] = y;
}
void GLTransform3D::setScale(GLfloat x, GLfloat y, GLfloat z) {
    _transformMat[0] = x;
    _transformMat[5] = y;
    _transformMat[10] = z;
}
void GLTransform3D::setPosition(GLfloat x, GLfloat y) {
    _transformMat[12] = x;
    _transformMat[13] = y;
}
void GLTransform3D::move(GLfloat x, GLfloat y) {
    _transformMat[12] += x;
    _transformMat[13] += y;
}

void GLTransform3D::setPosition(GLfloat x, GLfloat y, GLfloat z) {
    _transformMat[12] = x;
    _transformMat[13] = y;
    _transformMat[14] = z;
}

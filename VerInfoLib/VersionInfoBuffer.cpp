...
CVersionInfoBuffer::CVersionInfoBuffer(const CVersionInfoBuffer& buf) : m_dwBufSize(buf.m_dwBufSize), m_dwPosition(buf.m_dwPosition)
{
	m_lpData = (LPBYTE) new BYTE[m_dwBufSize];
	memcpy(m_lpData, buf.m_lpData, m_dwBufSize);
}
